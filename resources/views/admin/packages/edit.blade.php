<x-app-layout>
    ...
    <div
        x-data="packageForm({
            weekDays: {{ old('week_days', $package->week_days) }},
            rows: @js(old('rows', $package->trainerCategories->map(fn ($c) => ['trainer_category_id' => (string) $c->id, 'sessions' => $c->pivot->sessions])->values()->all() ?: [['trainer_category_id' => '', 'sessions' => '']])),
        })"
        class="max-w-2xl"
    >
        <x-card>
            <form method="POST" action="{{ route('admin.packages.update', $package) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <x-textarea-input id="description" name="description" rows="2" class="mt-1">{{ old('description', $package->description) }}</x-textarea-input>
                ...
                <div>
                    ...
                    <div class="mt-2 flex gap-4">
                        <label class="flex items-center gap-2 text-sm">
                            <input type="radio" name="week_days" value="3" x-model.number="weekDays" class="text-primary-600 focus:ring-primary-500" {{ old('week_days', $package->week_days) == 3 ? 'checked' : '' }}>
                            3 Days
                        </label>
                        <label class="flex items-center gap-2 text-sm">
                            <input type="radio" name="week_days" value="6" x-model.number="weekDays" class="text-primary-600 focus:ring-primary-500" {{ old('week_days', $package->week_days) == 6 ? 'checked' : '' }}>
                            6 Days Weekly
                        </label>
                    </div>
                ...
                <div>
                    ...
                    <x-text-input id="trial_sessions_count" type="number" min="1" max="10" name="trial_sessions_count" class="mt-1 w-32" value="{{ old('trial_sessions_count', $package->trial_sessions_count) }}" />
                ...
                <div class="flex items-center gap-3 pt-2">
                    <x-primary-button type="submit" ::disabled="isOverLimit">Save Changes</x-primary-button>
