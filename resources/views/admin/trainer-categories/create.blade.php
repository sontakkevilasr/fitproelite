<x-app-layout>
    <x-slot name="header">
        <x-page-header title="New Trainer Category" />
    </x-slot>

    <x-card class="max-w-xl">
        <form method="POST" action="{{ route('admin.trainer-categories.store') }}" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <x-textarea-input id="description" name="description" rows="3" class="mt-1">{{ old('description') }}</x-textarea-input>
                <x-input-error :messages="$errors->get('description')" class="mt-2" />
            </div>

            <div class="flex items-center gap-2">
                <x-checkbox id="is_assessment_category" name="is_assessment_category" value="1" :checked="old('is_assessment_category')" />
