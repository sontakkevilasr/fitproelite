<x-app-layout>
    <x-slot name="header">
        <x-page-header title="New User" subtitle="Create a login for an admin, counsellor, or trainer." />
    </x-slot>

    <x-card class="max-w-2xl">
        <form method="POST" action="{{ route('admin.users.store') }}" enctype="multipart/form-data" x-data="{ role: '{{ old('role', 'counsellor') }}' }" class="space-y-5">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <x-text-input id="email" type="email" name="email" class="mt-1" value="{{ old('email') }}" required />
            ...
            <div x-show="role === 'trainer'" x-cloak class="space-y-5 border-t border-gray-100 pt-5">
                    ...
                    <div class="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
                        @foreach($categories as $category)
                            <label class="flex items-center gap-2 text-sm text-gray-700">
                                <x-checkbox name="trainer_category_ids[]" value="{{ $category->id }}" :checked="in_array($category->id, old('trainer_category_ids', []))" />
                ...
                <div>
                    ...
                    <x-text-input id="session_duration_minutes" type="number" name="session_duration_minutes" class="mt-1" value="{{ old('session_duration_minutes', 60) }}" />
                </div>

                <div>
                    <x-input-label for="bio" value="Bio" />
                    <x-textarea-input id="bio" name="bio" rows="3" class="mt-1">{{ old('bio') }}</x-textarea-input>
                </div>
